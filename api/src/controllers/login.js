  ...
  try {
    ...
    */
    ...
    if (!isMatch) {
      ...
      // đăng nhập thành công => tạo mã refresh token
...
const postRefreshToken = async (req, res, next) => {
  try {
    ...
    return res.status(401).json({ message: "Unauthorized lol", error });
  }
};
...
const postLogout = async (req, res, next) => {
  try {
    ...
    if (express().get("env") === "production") accessToken = req.body.token;
